...
api.validateEvent = (event, validatorConfig, callback) => {
  // create an array of proof objects to check
  let proofObjects =
    (event.type === 'WebLedgerEvent') ? event.operation : [event];

  async.every(proofObjects, (proofObject, callback) => {
    const proofCopy = bedrock.util.clone(proofObject);
    const allProofs = (proofCopy.signature) ? [].concat(proofCopy.signature) :
      [].concat(proofCopy.proof);
    const eqSigs = allProofs.filter(s =>
    ...
    proofCopy.signature = eqSigs[0];
  
    equihashSigs.verify(proofCopy, (err, result) => {
      if(err) {
        return callback(err);
      }
      if(!result) {
